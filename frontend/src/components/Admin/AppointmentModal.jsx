// src/components/Admin/AppointmentModal.jsx (VERSÃO LIMPA, SEM TOKEN MANUAL)
...
export default function AppointmentModal({ onClose, onAppointmentCreated }) {
 	...
 	useEffect(() => {
 		...
 		api.get('/admin/clients').then(res => setUsers(res.data));
 	...
 	useEffect(() => {
 		if (selectedDate && selectedService && selectedProfessional) {
 			// Esta chamada é pública, nunca precisou de token
 	...
 	const handleCreateClient = async () => {
 		...
 		if (!emailRegex.test(email)) {
 			setError('Por favor, insira um formato de email válido (ex: cliente@email.com).');
 			return;
 		}

 		try {
 			const senhaPadrao = 'mudar123';
 			// A rota de registro é pública, não precisa de token
 			...
 			toast.success(`Cliente criado! A senha temporária é: ${senhaPadrao}`);
